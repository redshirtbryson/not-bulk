...
- **Q11a. Connection pooling:** worker parallelism is the scale path (N stateless workers competing via SKIP LOCKED), so Postgres connections are pooled from day one — sized pools in both runtimes (pg.Pool / psycopg pool), with pgbouncer added in front when worker count grows past the local default.
